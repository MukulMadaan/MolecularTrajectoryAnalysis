...
class ReadFiles(constants: CONSTANTS) {
  ...
  /**
   * This function to read all topology files
   *
   * @return ArrayBuffer of topology files
   */
  ...
  def setAtomConstants() = {
    ...
    constants.set_totalAtomCount(atomArray.length)
    ...
    constants.set_NonWaterAtomCount(residueCounter)
